...
1. Navigate to the MySQL project directory:
   ```bash
   cd ./src/Aiursoft.MusicTools.MySql/
   ```
...
```bash
dotnet ef migrations remove --context "MySqlContext" -s ../Aiursoft.MusicTools/Aiursoft.MusicTools.csproj
```

## After Creating Migrations

After creating migrations for MySQL, remember to:
1. Create the corresponding migration for SQLite (see `../Aiursoft.MusicTools.Sqlite/HOW_TO_ADD_MIGRATIONS.md`)
